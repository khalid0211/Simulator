...
import {
  ...
  FastForward, Rewind, BookOpen, FileText, Loader,
} from "lucide-react";
...
function newSim(cfg) {
  ...
  const sim = {
    ...
    status: "running",
    config: { fundingProfile, budgetTightness, politicalProjects, concurrentCap, approvalLag, riskMultiplier, blindAlignment, blindScore },
  };
  return sim;
}
      ...
      <div style={{ width: "min(600px, 96vw)" }}>
        ...
        <Panel style={{ padding: 22, marginBottom: 14 }}>
          ...
          <input value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Punjab ADP dry-run"
            style={{ width: "100%", boxSizing: "border-box", marginTop: 6, marginBottom: 20, padding: "10px 12px", borderRadius: 8, background: T.panel2, border: `1px solid ${T.line}`, color: T.text, fontSize: 14, outline: "none" }} />
...
function AvailableTable({ sim, onAdd }) {
  ...
  return (
    <div style={{ minHeight: "100vh", padding: "32px 20px", display: "grid", placeItems: "start center" }}>
      <div style={{ width: "min(840px, 96vw)" }}>
        ...
        <p style={{ color: T.muted, margin: "0 0 20px" }}>{sim.name} · {sim.annualRate ? (sim.annualRate * 100).toFixed(1) : 0}% inflation</p>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "minmax(340px, 420px) 1fr", gap: 16, padding: 16, alignItems: "start" }} className="sim-grid">
        ...
        <div>
          ...
          <Section title="Available pool" count={sim.projects.filter((p) => p.state === "available").length} color={T.available}>
            <AvailableTable sim={sim} onAdd={(id) => commit((n) => addProject(n, id))} />
          </Section>
        </div>
